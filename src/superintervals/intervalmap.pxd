...
cdef class IntervalMap:
    ...
    # Set-like operations (return new IntervalMap instances, already built)
    cdef list _sorted_intervals(self)
    cpdef merge_overlaps(self, combine= *)
    cpdef union_with(self, IntervalMap other, combine= *)
    cpdef intersection(self, IntervalMap other, combine= *)
    cpdef difference(self, IntervalMap other)
    cpdef symmetric_difference(self, IntervalMap other)
    cpdef gaps(self, int lo, int hi, object fill= *)
    cpdef span(self)
