...
class SuperIntervals {
    ...
    virtual void index(bool use_linear=false) {
        if (starts.size() == 0) {
            return;
        }
...
public:
    ...
    void index(bool use_linear=false) override {
        ...
        if (!use_linear) {
            ...
            this->extent = this->ends;
            for (size_t i=0; i < this->ends.size() - 1; ++i) {
                S e = this->ends[i];
                for (size_t j=i + 1; j < this->ends.size(); ++j) {
                    if (this->ends[j] >= this->ends[i]) {
                        break;
                    }
                    this->branch[j] = i;
                    if (e > this->extent[j]) {
